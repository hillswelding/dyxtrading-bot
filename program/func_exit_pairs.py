...
def get_order_book(client, market):
  print(f"Gettting Order Book for {market}")
  orderbook = client.public.get_orderbook(market=market)
  time.sleep(0.2)
  print(f"ASKS :{orderbook.data['asks'][0]} BIDS :{orderbook.data['bids'][0]}")
...
def manage_trade_exits(client):
  ...
  for position in open_positions_dict:
    ...
    if not check_m1 or not check_m2 or not check_live:
      print(f"MISS-MATCH POSITIONS - Warning: Not all open positions match exchange records for {position_market_m1} and {position_market_m2}")
      continue
    ...
    if CLOSE_AT_ZSCORE_CROSS:
      ...
      if z_score_level_check and z_score_cross_check:
        print(f"Z Score Current: {z_score_current} - Traded: {z_score_traded} - is at a level to CLOSE the TRADE for {position_market_m1} and {position_market_m2}")
