...
def get_candles_recent(client, market):
  ...
  for candle in candles.data["candles"]:
    close_prices.append(candle["close"]) # Get ASK and BUY using Quote..
...
def get_candles_historical(client, public_client, market):
  ...
  for timeframe in ISO_TIMES.keys():
    ...
    # Get data
    candles = public_client.public.get_candles(
      market=market,
      resolution=RESOLUTION,
      from_iso=from_iso,
      to_iso=to_iso,
...
def construct_market_prices(client, public_client):
  ...
  close_prices = get_candles_historical(client, public_client, tradeable_markets[0])
  ...
  for market in tradeable_markets[1:]:
    close_prices_add = get_candles_historical(client, public_client, market)
