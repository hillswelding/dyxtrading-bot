...
class BotAgent:
  ...
  def check_order_status_by_id(self, order_id):
    ...
    if order_status == "CANCELED":
      print(f"{self.market_1} vs {self.market_2} - Order cancelled... {order_id} CANCELED")
    ...
    if order_status != "FAILED":
      ...
      # Guard: If order cancelled move onto next Pair
      if order_status == "CANCELED":
        print(f"{self.market_1} vs {self.market_2} - Order cancelled... {order_id} FAILED")
        self.order_dict["pair_status"] = "FAILED"
        time.sleep(2)
        return "failed"
  ...
  def open_trades(self):
    ...
    # Print status
    print("---")
    print(f"{self.market_1}: Placing BASE order...")
    ...
    try:
      ...
      self.order_dict["pair_status"] = "ERROR"
      self.order_dict["comments"] = f"Market BASE {self.market_1}: , {e}"
    ...
    print("---")
    print(f"{self.market_2}: Placing QUOTE order...")
    ...
    try:
      ...
      self.order_dict["pair_status"] = "ERROR"
      self.order_dict["comments"] = f"Market QUOTE {self.market_2}: , {e}"
      return self.order_dict
    ...
    if order_status_m2 != "live":
      ...
      try:
        ...
        if order_status_close_order != "FILLED":
          ...
          # Send Message
          send_message(f'Failed to execute. Code red. Error code: 100 - {time.strftime("%H:%M:%S",time.localtime())}')

          # ABORT
          exit(1)
      except Exception as e:
        self.order_dict["pair_status"] = "ERROR"
        self.order_dict["comments"] = f"Close Market BASE {self.market_1}: , {e}"
        print("ABORT PROGRAM")
        print("Unexpected Error")
        print(order_status_close_order)

        # Send Message
        send_message(f'Failed to execute. Code red. Error code: 101 - {time.strftime("%H:%M:%S",time.localtime())}')
